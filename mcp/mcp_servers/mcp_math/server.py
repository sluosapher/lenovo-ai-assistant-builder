...
# Message templates
SERVER_MESSAGES = {
    'server_start': 'math MCP server starting' + (' on port {port}' if 'stdio' != 'stdio' else ''),
    'server_started': 'math MCP server started successfully' + (' on port {port}' if 'stdio' != 'stdio' else ''),
    'server_stopped': 'math MCP server stopped',
    'server_start_fail': 'Failed to start math MCP server',
    ...
    'status_running': 'math MCP server is running',
    'status_stopped': 'math MCP server is not running',
    'ping_success': 'Server is responding',
    'ping_fail': 'Server is not responding',
    'interrupt': 'Operation cancelled by user',
    'error': 'Error: {error}',
    'version': 'math MCP Server v{version}',
}
...
def create_response(success, message, data=None, exit_code=0, current_port=None, current_protocol=None):
    ...
    response = {
        ...
        "protocol": current_protocol or math_PROTOCOL,
    }
...
# Generated by McpServerGen.py on 2025-09-02 10:57:13
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
math_PORT = int(os.getenv('MCP_MATH_PORT', 7905)) if 'stdio' != 'stdio' else None
math_PROTOCOL = os.getenv('MCP_MATH_PROTOCOL', 'stdio')
...
async def add(a: int, b: int) -> str:
    ...
    Returns:
        str: A formatted string with the addition result
    """
    try:
        return f"The sum of {a} and {b} is {a + b}."
    except Exception as e:
        raise RuntimeError(f"Error in add: {str(e)}")
...
async def subtract(a: int, b: int) -> str:
    ...
    Returns:
        str: A formatted string with the subtraction result
    """
    try:
        return f"The difference between {a} and {b} is {a - b}."
    except Exception as e:
        raise RuntimeError(f"Error in subtract: {str(e)}")
...
async def multiply(a: int, b: int) -> str:
    ...
    Returns:
        str: A formatted string with the multiplication result
    """
    try:
        return f"The product of {a} and {b} is {a * b}."
    except Exception as e:
        raise RuntimeError(f"Error in multiply: {str(e)}")
...
async def divide(a: int, b: int) -> str:
    ...
    Returns:
        str: A formatted string with the division result
    """
    try:
        if b == 0:
        ...
        raise RuntimeError(f"Error in divide: {str(e)}")
...
def start_server(port=None, protocol=None):
    ...
    if (protocol == 'stdio' and mcp is None) or (protocol != 'stdio' and port != math_PORT):
        # Create fresh FastMCP instance with current port and protocol
        if protocol == 'stdio':
            mcp = FastMCP("math")
        else:
            mcp = FastMCP("math", port=port)
                    ...
                    try:
                        ...
                        if proc.info['name'] and 'python' in proc.info['name'].lower():
                            ...
                            if (cmdline and 
                                ...
                                any('math' in str(arg).lower() for arg in cmdline)):
...
def is_server_running():
    ...
    return _server_running and is_port_in_use(math_PORT)
...
Examples:
  ...
  python server.py version
    ...
    try:
        ...
        current_port = math_PORT
        current_protocol = math_PROTOCOL
        if args.port:
            current_port = args.port
            os.environ[f'MCP_MATH_PORT'] = str(current_port)
        if args.protocol:
            current_protocol = args.protocol
            os.environ[f'MCP_MATH_PROTOCOL'] = current_protocol
        ...
        if current_protocol == 'stdio':
            ...
            mcp = FastMCP("math", port=current_port)
        ...
        current_port_safe = locals().get('current_port', math_PORT)
        current_protocol_safe = locals().get('current_protocol', math_PROTOCOL)
        response, exit_code = create_response(
            True, 
            format_message('interrupt'),
